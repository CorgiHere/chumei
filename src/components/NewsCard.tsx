  ...
  return (
    <article className="card p-5">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        {post.pinned && (
          <span className="rounded-full bg-[var(--color-black)] px-2 py-0.5 text-xs font-bold text-white">
            置頂
          </span>
        )}
      </div>
      ...
      <p className="mt-2 text-sm text-[var(--color-gray)]">{post.summary}</p>
      <time
        dateTime={post.publishedAt}
        className="mt-3 block text-xs text-[var(--color-gray)]"
      >
